# The ELT files  (*EL.py*, *TermCandidate.py*) originate from [https://github.com/Majeed7/EntityLinking](https://github.com/Majeed7/EntityLinking). Please refer to this repo for the full ELT code. The files included here are slightly adapted, as noted in the code. The changes are minimal:
# - return exact matches immediately
# - return multiple matches per fragment if applicable (we do this as we filter out more items later by filtering out Clinical Concepts only).
    ...
    def __reduceCandidates(self, allCandidates, db):
        ...
        def combine_candidates(candidate1, candidate2, db):
            ...
            if new_terms != []:
                ...
                #candidate1.variations = [] # keep cand1, potential dubplicate return but we filter out later [R]
                candidate2.variations = sorted(candidate2.variations, key=len, reverse=True)
...
if __name__ == "__main__":
    ...
    doc = "Zelden (0,01-0,1%): Spierzwakte. Huiduitslag, jeuk, urticaria,"

    el = EntityLinking(doc)

    for candidate in el.AllCandidates:
        print("candidate", str(candidate), ":")
        for s in candidate.SimilarEntities:
            print(str(s), s.ConceptId)
